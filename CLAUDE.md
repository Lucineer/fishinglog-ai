# CLAUDE.md — Fishinglog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Fishinglog Ai
- **Role**: FishingLog.ai
- **URL**: https://fishinglog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/fishinglog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/fishinglog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://fishinglog-ai.casey-digennaro.workers.dev/health
curl -s https://fishinglog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=fishinglog-ai
...
- **vessel.json capabilities**: chat, species, weather
...
1. Keep the vessel small. ~102 lines is the current size.
